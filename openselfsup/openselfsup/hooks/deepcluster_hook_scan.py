...
class DeepClusterHookScan(Hook):
    """Hook for SCAN.
    ...
    def deepcluster(self, runner):
        # step 1: get features
        runner.model.eval()
        features, gt_labels = self.extractor(runner)
        if np.any(np.isnan(features)):
            raise Exception("Has nan in features when extract.")
        ...
        if self.init_memory:
            if np.any(np.isnan(features)):
                raise Exception("Has nan in features when init.")
            runner.model.module.memory_bank.init_memory(features, new_labels, gt_labels)
