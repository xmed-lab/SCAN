...
class WideResNet(nn.Module):
    ...
    def forward(self, x, target= None, mixup=False, mixup_hidden=True, mixup_alpha=None , lam = 0.4):
        ...
        return tuple([out])#, out1
        
    def init_weights(self, pretrained=None):
        if isinstance(pretrained, str):
            logger = get_root_logger()
            load_checkpoint(self, pretrained, strict=False, logger=logger, revise_keys=[(r'^backbone\.', '')])
...
def wrn28_10(num_classes=200 , loss_type = 'dist'):
    model = WideResNet(depth=28, widen_factor=10, num_classes=num_classes, loss_type = loss_type , per_img_std = False, stride = 1, dropRate = 0.5 )
    return model
