...
class distLinear(nn.Module):
    def __init__(self, indim, outdim):
        ...
        if outdim <=200:
            self.scale_factor = 4; #a fixed scale factor to scale the output of cos value into a reasonably large input for softmax, for to reproduce the result of CUB with ResNet10, use 4. see the issue#31 in the github 
        else:
...
## Meta-Derm 2020 paper: Conv6:
class ConvNetPaper(nn.Module):
    def __init__(self, depth, flatten = True):
        super(ConvNetPaper,self).__init__()
        trunk = []
        for i in range(depth):
            indim = 3 if i == 0 else 32
            outdim = 32
            B = ConvBlock(indim, outdim, pool = True ) #only pooling for fist 4 layers
            trunk.append(B)
...
class ResNet(nn.Module):
    ...
    def __init__(self,block,list_of_num_layers, list_of_out_dims, flatten = True):
        ...
        if flatten:
            #avgpool = nn.AvgPool2d(7) # for input size 224
            avgpool = nn.AvgPool2d(3) # for input size 80
